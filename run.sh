...
docker run --rm --name presidio-anonymizer-image --network $NETWORKNAME -d -p 3002:3002 -e GRPC_PORT=3002 ${DOCKER_REGISTRY}/presidio-anonymizer-image:${PRESIDIO_LABEL}
docker run --rm --name presidio-ocr --network $NETWORKNAME -d -p 3003:3003 -e GRPC_PORT=3003 ${DOCKER_REGISTRY}/presidio-ocr:${PRESIDIO_LABEL}
